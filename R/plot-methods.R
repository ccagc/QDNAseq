...
setMethod("plot", signature(x="QDNAseqSignals", y="missing"), 
    function (x, y, 
        main=NULL, includeReadCounts=TRUE, logTransform=TRUE, scale=TRUE, 
        sdFUN="sdDiffTrim", delcol="darkred", losscol="red", gaincol="blue", 
        ampcol="darkblue", pointcol="black", segcol="chocolate", misscol=NA,
        xlab="chromosomes", ylab=NULL, ylim=NULL, xaxt="s", yaxp=NULL,
        showDataPoints=TRUE, showSD=TRUE, pointpch=1, pointcex=.1, 
        doSegments=TRUE, doCalls=TRUE, ... ) {
        ...
        if ("copynumber" %in% assayDataElementNames(x)) {
            copynumber <- 
                assayDataElement(x, "copynumber")[condition, , drop=FALSE]
            if (is.null(ylab))
        ...
        if (!scale) {
            ...
            starts <- 
                cumsum(c(1, rle.chrom$lengths[-length(rle.chrom$lengths)] + 1))
            ...
            startI <- c(1, 
                cumsum(rle.chrom$lengths[-length(rle.chrom$lengths)] ) + 1 )
            ...
            mask <- sapply(1:length(startI), function(i) { 
                sum(condition[startI[i]:endI[i]]) != 0
            })
...
setMethod("frequencyPlot", signature=c(x="QDNAseqCopyNumbers", y="missing"),
    function(x, y, main="Frequency Plot", losscol="red", gaincol="blue",
        ...
        axis(side=2, at=c(-1, -0.5, 0, 0.5, 1), 
            labels=c("100 %", " 50 %", "0 %", "50 %", "100 %"), las=1)
...
setMethod("isobarPlot", signature=c(x="QDNAseqReadCounts", y="missing"),
    definition=function(x, y, main=NULL, nlevels=20L, 
        ...
        median.signal <- aggregate(signal[condition, ], 
        ...
        for (i in seq_len(ncol(x))) {
            ...
            reads <- 
                sum(assayDataElement(x, "counts")[condition, i], na.rm=TRUE)
...
plotZoom <- function(obj, scale=TRUE, ...) {
    locator(2) -> sel
    ...
    # Selecting last plotted sample in object.
    plot(obj[zoom,ncol(obj)], scale=scale, ...) 
