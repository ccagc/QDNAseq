...
aberrations with CGHcall or cutoffs have been implemented for convenience.
...
It should be noted that CGHcall (which \code{callBins()} uses by default) was
developed for the analysis of sets of cancer samples. It is based on a mixture
model, and when there are not enough aberrations present in the data, model
fitting can fail. This can happen especially with non-cancer samples, and/or
when analyzing individual cases instead of larger data sets.

If CGHcall fails, \code{callBins()} can also perform simple cutoff-based
calling by setting parameter \code{method="cutoff"}. The default cutoff values
are based on the assumption of uniform cell populations, and in case of cancer
samples will most likely need calibration by adjusting parameter \code{cutoffs}.
