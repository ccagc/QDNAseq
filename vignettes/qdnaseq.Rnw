...
readCounts <- binReadCounts(bins) # all files ending in .bam from the current working directory
# or
readCounts <- binReadCounts(bins, bamfiles='tumor.bam') # file tumor.bam from the current working directory
# or
readCounts <- binReadCounts(bins, path='tumors') # all files ending in .bam from the subdirectory tumors of the current working directory
@
...
<<reg>>=
#LGG150 <- correct(LGG150)
@

Perform median normalization.

<<reg>>=
#LGG150 <- normalize(LGG150)
@
...
plot(LGG150)
#highlightFilters(LGG150, mappability=50, residual=1)
@
...
<<reg>>=
#LGG150 <- applyFilters(LGG150, mappability=50, residual=1)
@
...
<<reg>>=
#LGG150 <- postsegnormalize(LGG150)
#cal <- CGHcall(LGG150)
#LGG150 <- ExpandCGHcall(cal, LGG150)
@
