...
To apply filtering, plot the profile and highlight bins to be filtered out for evaluation. By default, bins with mappabilities lower than 50 and the 1000 Genomes residuals larger than 1 standard deviations are filtered out.

% png? https://stat.ethz.ch/pipermail/r-help/2009-November/218795.html

<<label=filters,include=FALSE>>=
plot(LGG150)
highlightFilters(LGG150, mappability=50, blacklist=0, residual=4, bases=0)
@
...
<<reg>>=
LGG150 <- applyFilters(LGG150, residual=4, blacklist=0,
  mappability=50, bases=0)
@
...
# load data for the 1000 Genomes (or similar) data set
tg <- binReadCounts(bins,
  path='/path/to/1000Genomes/bam/files')
# perform correction for GC content and mappbility
tg <- correctBins(tg, storeResiduals=TRUE)
# calculate median residual across samples
residual <- apply(Biobase::assayDataElement(tg, 'residuals'), 1,
  median, na.rm=TRUE)
# transform the median residuals to z-scores
bins$residual <- scale(residual)[, 1]

# generate an AnnotatedDataFrame object
